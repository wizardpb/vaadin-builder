...
/**
 * <p>A {@link FieldGroupFactory} takes an instance of a {@link Layout} as an attribute and attaches a {@link FieldGroup} to it. This then becomes
 * a container for further nodes, usually {@link com.vaadin.ui.Field}s. When building contained {@link com.vaadin.ui.Field}s, the {@link FieldFactory} looks for
 * a {@link FieldGroup} on a containing node, and arranges for these {@link com.vaadin.ui.Field}s to be bound using that {@link FieldGroup}.
 * The arrangement allows forms of varying complexity to be build using the full range of layouts and containers, as well as multiple field groups.</p>
 *
 * <p>Since the {@link FieldGroupFactory} returns a {@link Layout}, but is not a {@link LayoutFactory}, it must appear as such to the builder for the rest of
 * the node building process. It does this by masquerading as a {@link LayoutFactory} by delegating to an instance of such that would have built the
 * {@link Layout} passed in. It does this using a {@link Delegate}, and resetting the delegate factory dynamically on every node call.</p>
 *
